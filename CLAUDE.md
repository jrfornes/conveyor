...
```
bin/conveyor            operator CLI: init | start [--no-setup] | stop [--now] | setup | task [--delete]
                        | status | log | cost | resume
...
- **Isolation**: `conveyor start` creates `.worktrees/<role>` on branch `conveyor-<role>`, writes the concatenated constitution + project + role file + a generated `## Handoff contract` (derived from `conveyor.conf`, not the role file) + assigned skills to `.cursor/rules/conveyor-role.mdc` in each worktree, and installs the shared `commit-msg` byline hook. Main checkout is the integration tree; no agent runs there.
...
- **Ceilings** park to `needs-human/` with a `reason` file: `max-retries`, `max-minutes`, `max-attempts`, `max-tokens`, `merge-conflict`, `untracked-collision`, `multiple-handoffs`, `no-rules`, `no-agent`, `no-task-file`, `setup-failed`, `done-command`. `conveyor resume` is the only way out.
- **Worktree setup** (`[global] worktree_setup`, optional): one opaque operator command that makes a role's tree runnable. Runs at `conveyor start` and in the loop right after a merge that moved a `worktree_setup_paths` entry, keyed on a content stamp (blob oids at HEAD + the command) in `roles/<role>/setup.stamp`. Written only on exit 0; failure dies at `start` and parks `setup-failed` in the loop. Unset = the feature does not exist.
