...
- **Ceilings** park to `needs-human/` with a `reason` file: `max-retries`, `max-minutes`, `max-attempts`, `merge-conflict`, `multiple-handoffs`, `no-rules`, `no-agent`, `done-command`. `conveyor resume` is the only way out.
