...
.cursor/rules/conveyor-role.mdc         generated by conveyor start; gitignored
                                        (constitution + project.md + role file + generated
                                        handoff contract + assigned skills, in that order; the
                                        handoff contract is generated from conveyor.conf, see §6.3.
...
`conveyor start` composes that file in five parts, in order: the constitution files, `project.md`, the role file (`roles/<role>.md`), a generated `## Handoff contract` section, and — when the role has assigned skills — a `## Assigned skills` block. The handoff contract is **generated from `conveyor.conf`**, not written into the role file: it restates the role's permitted routes (the same triples `handoff.sh` derives, §3.2), so it is always correct for the active belt and reorders when the workflow does. The role file carries only craft (`## Owns`, `## Does not own`, and role-specific guidance); a role file that still contains a hand-written `## Handoff contract` is accepted, but the generated block overrides it and `conveyor start` warns. Rendered for the two-pack `coder → reviewer`, the `coder` block is:
...
You receive `ready` from operator (new task) and `findings` from reviewer (rework).
...
The last role's block instead lists the `findings` and `pass` rows and states that `findings` requires a commit you made (an empty commit carrying the findings list is the expected form); a gated role's block adds that its `ready` is held in `.conveyor/approvals/pending/` for the operator.
